...
export default function UsersMenu() {
  ...
  const userId = localStorage.getItem("userInfo") ? JSON.parse(localStorage.getItem("userInfo")).name : '';
  ...
  const handleLogOut = async () => {
    try {
      ...
      const response = await logout(userInfo.id);
      alert('로그아웃');
      navigate('/');
    ...
    <>
      ...
      <div className="p-2">
        <Link
          ...
          to={`/${userId}/info`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-black dark:text-blue-700" fill="none" viewBox="0 0 20 20" stroke="currentColor">
      ...
      <div className="p-2">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-black dark:text-blue-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
